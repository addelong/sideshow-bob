...
print_help() {
    cat << 'EOF'
Ouroboros Loop - Self-Improving Iterative Development
...
DESCRIPTION:
    Unlike ralph-wiggum which feeds the same prompt repeatedly, Ouroboros evolves
    the prompt each iteration based on learnings from the previous attempt.
...
EXAMPLES:
    ...
    /ouroboros-loop "Build a REST API with CRUD operations and tests" \
        --completion-promise "ALL_TESTS_PASS"

    # With iteration limit as safety net
    /ouroboros-loop "Implement OAuth2 authentication flow" \
        --max-iterations 30 \
...
echo "Ouroboros loop initialized!"
echo ""
